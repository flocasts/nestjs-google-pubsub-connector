...
import { ClientProxy, Deserializer, ReadPacket, Serializer, WritePacket } from '@nestjs/microservices';
...
export class ClientGooglePubSub extends ClientProxy {
    ...
    protected initializeDeserializer(options?: GooglePubSubOptions): void {
        this.deserializer = options?.deserializer ? options.deserializer : new GooglePubSubMessageDeserializer();
    }

    protected initializeSerializer(options?: GooglePubSubOptions): void {
        this.serializer = options?.serializer ? options.serializer : new GooglePubSubMessageSerializer();
    }
